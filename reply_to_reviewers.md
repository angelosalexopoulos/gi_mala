...
- For the variance reduction tables (Tables 6–8, Section 5.2), we have added `run_VR_experiments.m` and three new post-processing scripts. For the GP experiments, `make_results_VR_binaryclassification.m` and `make_results_VR_girolami.m` run 100 independent repeats for multiple chain lengths and compute the variance ratio ranges reported in Tables 7, 8, and 12. For the Bayesian logistic regression VR experiment (Table 6), we have added `make_table6_logistic.R`, an R script that implements the two-CV GI-MALA variance reduction for the Heart and Australian datasets across all four chain lengths (n ∈ {1k, 10k, 50k, 200k}).
